/**
 * seeks to extend the BookReader prototype to become aware of Islandora Compound Objects.
 *
 * Islandora Compound Objects are a RDFS that describes an object's relationship to
 * any items included on/with that object at a given sequence/position, their relation
 * to one another, and their relation back to the parent.
 *
 * e.g.: a playbill with a ticket stub located on page @ position 5, a scrapbook with
 * ephemera on several pages, sometimes more than one per page, etc...
 *
 * @author Benjamin Rosner (github: br2490) - 7/17/2017 first commit.
 *
 * @LICENSE adopts least restrictive: Islandora Internet Archive BookReader, or Open Library Internet Archive BookReader
...
(function ($) {
    ...
    BookReader.prototype.init = function () {
        // This is first so we can set the initial display mode for compound objects. Otherwise you can skip it.
        this.getCompoundInformation();

        this.bookReaderInitialMode = this.settings.bookReaderInitialMode || 0;
        if (this.bookReaderInitialMode > 0) {
    ...
    BookReader.prototype.canSwitchToMode = function (mode) {
        if (this.bookReaderOnCompound) {
            // compound objects are kept within 1up and thumbs! and only some scrapbooks are allowed thumbs!
     ...
     * - compoundInclusionObjects: jQuery object of all compound objects in the Compound Block.
     * - bookReaderOnCompound: true|false
     * - bookReaderOnInclusion: true|false - Are we on an active inclusion? (i.e., not the parent, but a child object.)
     * - compoundInclusionPageNum: inclusion page number. (not the same as book location/page number.)
     * - compoundActiveInclusion: jQuery object of ACTIVE inclusion compound objects in the Compound Block.
    ...
    BookReader.prototype.getCompoundInformation = function () {
        ...
        // We should already know this is true, but in case the block is preset but no objects are included.
        ...
        // Are we on an active inclusion? (i.e., not the parent, but a child object.)
        ...
        if (this.bookReaderOnInclusion) {
            ...
            // Why am I relying on the breadcrumbs?  Because we can?  Sanity check, please.
    ...
    /**
     * Method to change the hash location if we're overriding the initially set display mode.
     *
     * This happens before final init of the BookReader, so that paramsFromFragment() can consume our hash
    ...
    BookReader.prototype.updateCompoundBlockLocation = function () {
        ...
        this.compoundBlock.offset({'top': reader.offset().top, 'left': this.compoundBlock.offset().left});
    };

    /**
     * Fires on page change to hide and show the appropriate inclusion objects, related titles, links, etc.
     *
     * This just sets a title and shows/hides inclusions as you navigate the book.
     *
     * @param index
     */
